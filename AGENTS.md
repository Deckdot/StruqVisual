# AGENTS.md — Struq Operating Guide (all agents)

This file is the operating contract for every AI agent working in this repo (Codex, Gemini, OpenCode, Cursor, etc.). It is identical in intent to `CLAUDE.md`.

> **MANDATORY — before anything else:** load the `master-orchestrator` skill. Depending on your runtime the skills are available at `.agents/skills/`, `.agent/skills/`, or `.claude/skills/` (identical mirrors). It classifies the task, picks the lane/skills, and decides what to read — do not pre-load docs.
...
- Canonical skills live in `.skillshare/skills/` — **edit skills there only**, then run `skillshare sync`. The mirrors are generated; never edit them.

## Verification Policy

`T0` typecheck · `T1` +lint · `T2` +verify:design · `T3` verify · `T4` verify:full (E2E). Use the lowest tier that fully covers the change. Details: `WORKFLOW.md`.
